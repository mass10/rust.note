...
impl Drop for ApplicationLifetime {
	fn drop(&mut self) {
		if self.exit_code != 0 {
			println!(
				"[ApplicationLifetime] QUIT IMMEDIATELY. (with exit_code: {})",
				self.exit_code
			);
		...
		println!(
			"[ApplicationLifetime] QUIT NATURALLY. (with exit_code: {})",
			self.exit_code
		);
	}
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
	let _longest_lifetime = MyLifetime {
		name: "longest".to_string(),
	};
	let mut _application_lifetime = ApplicationLifetime { exit_code: 0 };
	let _medium_lifetime = MyLifetime {
		name: "medium".to_string(),
	};
	let _shortest_lifetime = MyLifetime {
	...
	// 現在のミリ秒
	let current_millisec = std::time::SystemTime::now()
